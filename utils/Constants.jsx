export const NavList = [
  {
    id: 1,
    text: "Biz haqimizda",
    href: "/haqimizda",
  },
  {
    id: 2,
    text: "Shifokorlar",
    href: "/Shifokorlar",
  },
  {
    id: 3,
    text: "Aloqa",
    href: "/Aloqa",
  },
  {
    id: 4,
    text: "FAQ",
    href: "/Faq",
  },
];

export const navContact = [
  {
    id: 1,
    img: "fa fa-phone",
    text: "+998 99 231 2433",
    href: "tel:+998 99 231 2433",
  },
  {
    id: 2,
    img: "fa fa-envelope",
    text: "support@yangi-zamon.uz",
    href: "email:support@yangi-zamon.uz",
  },
];

export const HeaderList = [
  {
    id: 1,
    href: "/",
    text: "Bosh Sahifa",
  },
  {
    id: 2,
    href: "/haqimizda",
    text: "Biz Haqimizda",
  },
  {
    id: 3,
    href: "/Shifokorlar",
    text: "Shifokorlar",
  },
  {
    id: 4,
    href: "/vaqt",
    text: "Ish Vaqti",
  },
  {
    id: 5,
    href: "/narx",
    text: "Narxlar",
  },
  {
    id: 7,
    href: "/yangilik",
    text: "Yangiliklar",
  },
  {
    id: 6,
    href: "/kasallik",
    text: "Kassalik",
  },
];

export const social = [
  {
    id: 1,
    img: "icofont-facebook",
  },
  {
    id: 2,
    img: "icofont-google-plus",
  },
  {
    id: 3,
    img: "icofont-twitter",
  },
  {
    id: 4,
    img: "icofont-vimeo",
  },
  {
    id: 5,
    img: "icofont-pinterest",
  },
];

export const singleFooter = [
  {
    id: 1,
    img: "fa fa-caret-right",
    text: "Bosh sahifa",
  },
  {
    id: 2,
    img: "fa fa-caret-right",
    text: "Biz haqimizda",
  },
  {
    id: 3,
    img: "fa fa-caret-right",
    text: "Xizmatlar",
  },
  {
    id: 4,
    img: "fa fa-caret-right",
    text: "havolalar",
  },
];

export const singleFooter2 = [
  {
    id: 1,
    img: "fa fa-caret-right",
    text: "Maslahat",
  },
  {
    id: 2,
    img: "fa fa-caret-right",
    text: "Moliya",
  },
  {
    id: 3,
    img: "fa fa-caret-right",
    text: "Guvohlar",
  },
  {
    id: 4,
    img: "fa fa-caret-right",
    text: "FAQ",
  },
];

export const footerList = [
  {
    id: 1,
    day: "Dushanba - Juma ",
    time: "8.00-20.00",
  },
  {
    id: 2,
    day: "Shanba",
    time: "9.00-18.30",
  },
  {
    id: 3,
    day: "Dushanba - Shu kun ",
    time: "9.00-15.00",
  },
];

export const WhyChooseList = [
  {
    id: 1,
    img: "fa fa-caret-right",
    text: "Maecenas vitae luctus nibh.",
  },
  {
    id: 2,
    img: "fa fa-caret-right",
    text: "Duis massa massa.",
  },
  {
    id: 3,
    img: "fa fa-caret-right",
    text: "Aliquam feugiat interdum.",
  },
];

import pf1 from "../styles/img/pf1.jpg";
import pf2 from "../styles/img/pf2.jpg";
import pf3 from "../styles/img/pf3.jpg";
import pf4 from "../styles/img/pf4.jpg";

export const portfolioList = [
  {
    img: pf1,
  },
  {
    img: pf2,
  },
  {
    img: pf3,
  },
  {
    img: pf4,
  },
  {
    img: pf1,
  },
  {
    img: pf2,
  },
  {
    img: pf3,
  },
  {
    img: pf4,
  },
];

export const service = [
  {
    classList: "col-lg-4 col-md-6 col-12",
    classList2: "single-service wow fadeInUp",
    img: "icofont icofont-prescription",
    text: "Umumiy davolash",
  },
  {
    classList: "col-lg-4 col-md-6 col-12",
    classList2: "single-service wow fadeInUp",
    img: "icofont icofont-tooth",
    text: "Tishlarni oqartirish",
  },
  {
    classList: "col-lg-4 col-md-6 col-12",
    classList2: "single-service wow fadeInUp",
    img: "icofont icofont-heart-alt",
    text: "Yurak jarrohligi",
  },
  {
    classList: "col-lg-4 col-md-6 col-12",
    classList2: "single-service wow fadeInUp",
    img: "icofont icofont-listening",
    text: "Quloqni davolash",
  },
  {
    classList: "col-lg-4 col-md-6 col-12",
    classList2: "single-service wow fadeInUp",
    img: "icofont icofont-eye-alt",
    text: "Ko'rish muammolari",
  },
  {
    classList: "col-lg-4 col-md-6 col-12",
    classList2: "single-service wow fadeInUp",
    img: "icofont icofont-blood",
    text: "Qon quyish",
  },
];

export const AboutHeaderList = [
  {
    img: "icofont-tick-mark",
    text: "Yaxshiroq natijaga erishish uchun ilmiy ko'nikmalar",
  },
  {
    img: "icofont-tick-mark",
    text: "Aloqa olish uchun muloqot qobiliyatlari",
  },
  {
    img: "icofont-tick-mark",
    text: "Karyera haqida umumiy ma'lumot olish imkoniyati mavjud",
  },
  {
    img: "icofont-tick-mark",
    text: "Ish uchun yaxshi ish muhiti",
  },
  {
    img: "icofont-tick-mark",
    text: "Yaxshiroq natijaga erishish uchun ilmiy ko'nikmalar",
  },
  {
    img: "icofont-tick-mark",
    text: "Aloqa olish uchun muloqot qobiliyatlari",
  },
];

import doc1 from "../styles/img/team1.jpg";
import doc2 from "../styles/img/team2.jpg";
import doc3 from "../styles/img/team3.jpg";
import doc4 from "../styles/img/team4.jpg";

export const DoctorsList = [
  {
    link: "Korikga yozilish",
    text: "Neyroxirurg",
    title: "Collis Molate",
    img: doc1,
  },
  {
    link: "Korikga yozilish",
    text: "Neyroxirurg",
    title: "Domani Plavon",
    img: doc2,
  },
  {
    link: "Korikga yozilish",
    text: "Tish jarrohi",
    title: "John Mard",
    img: doc3,
  },
  {
    link: "Korikga yozilish",
    text: "Neyroxirurg",
    title: "Amanal Frond",
    img: doc4,
  },
  {
    link: "Korikga yozilish",
    text: "Neyroxirurg",
    title: "Collis Molate",
    img: doc1,
  },
  {
    link: "Korikga yozilish",
    text: "Neyroxirurg",
    title: "Domani Plavon",
    img: doc2,
  },
];

import team1 from "../styles/img/team1.jpg";
import team2 from "../styles/img/team2.jpg";
import team3 from "../styles/img/team3.jpg";
import team4 from "../styles/img/team4.jpg";

export const PricingList = [
  {
    name: "Collis Molate",
    job: "Neyroxirurg",
    img: team1,
  },
  {
    name: "Domani Plavon",
    job: "Neyroxirurg",
    img: team2,
  },
  {
    name: "John Mard",
    job: "Tish jarrohi",
    img: team3,
  },
  {
    name: "Amanal Frond",
    job: "Neyroxirurg",
    img: team4,
  },
];

import blogImg1 from "../styles/img/blog1.jpg";
import blogImg2 from "../styles/img/blog2.jpg";
import blogImg3 from "../styles/img/blog3.jpg";

export const BlogList = [
  {
    img: blogImg1,
    text: "Biz yangi mahsulotimizni e'lon qildik.",
  },
  {
    img: blogImg2,
    text: "Tish muammolarini hal qilishning eng yaxshi besh usuli.",
  },
  {
    img: blogImg3,
    text: "Biz yuqori darajadagi biznes yechimlarini taqdim etamiz.",
  },
];

export const newsData = [
  {
    img: blogImg1,
    text: "Biz yangi mahsulotimizni e'lon qildik.",
  },
  {
    img: blogImg2,
    text: "Tish muammolarini hal qilishning eng yaxshi besh usuli.",
  },
  {
    img: blogImg3,
    text: "Biz yuqori darajadagi biznes yechimlarini taqdim etamiz.",
  },
  {
    img: blogImg3,
    text: "Biz yuqori darajadagi biznes yechimlarini taqdim etamiz.",
  },
  {
    img: blogImg1,
    text: "Biz yangi mahsulotimizni e'lon qildik.",
  },
  {
    img: blogImg2,
    text: "Tish muammolarini hal qilishning eng yaxshi besh usuli.",
  },
];

export const tableData = [
  [
    { id: 1, time: "9.00" },
    { id: 2, name: "Dr. Tanner", prof: "Dermatologists" },
    { id: 3, name: "Dr. Kwak", prof: "Ear, Nose" },
    { id: 4, name: "Dr. Slaughter", prof: "Neurologist" },
    { id: 5, nobody: true },
    { id: 6, name: "Dr. Foley", prof: "Oncologist" },
    { id: 7, name: "Dr. Palmer", prof: "Maxine lowe" },
  ],
  [
    { id: 1, time: "12.00" },
    { id: 2, nobody: true },
    { id: 3, name: "Dr. Megahead", prof: "Orthopedics" },
    { id: 4, name: "Dr. Neupane", prof: "Pain Management" },
    { id: 5, name: "Dr. Breidin", prof: "Radiologist" },
    { id: 6, nobody: true },
    { id: 7, name: "Dr. Pipe", prof: "Surgeons" },
  ],
  [
    { id: 1, time: "15.00" },
    { id: 2, name: "Dr. Tanner", prof: "Dermatologists" },
    { id: 3, name: "Dr. Kwak", prof: "Ear, Nose" },
    { id: 4, nobody: true },
    { id: 5, name: "Dr. Slaughter", prof: "Neurologist" },
    { id: 6, name: "Dr. Foley", prof: "Oncologist" },
    { id: 7, nobody: true },
  ],
  [
    { id: 1, time: "12.00" },
    { id: 2, name: "Dr. Slaughter", prof: "Neurologist" },
    { id: 3, name: "Dr. Megahead", prof: "Orthopedics" },
    { id: 4, name: "Dr. Neupane", prof: "Pain Management" },
    { id: 5, name: "Dr. Breidin", prof: "Radiologist" },
    { id: 6, name: "Dr. Kwak", prof: "Ear, Nose" },
    { id: 7, name: "Dr. Pipe", prof: "Surgeons" },
  ],
];

export const postsData = [
  {
    id: 1,
    img: blogImg3,
    title: "Biz yangi mahsulotimizni e'lon qildik.",
    time: "Jan 11, 2022",
    comment: 35,
  },
  {
    id: 2,
    img: blogImg3,
    title: "Biz yangi mahsulotimizni e'lon qildik.",
    time: "Jan 11, 2022",
    comment: 35,
  },
  {
    id: 3,
    img: blogImg3,
    title: "Biz yangi mahsulotimizni e'lon qildik.",
    time: "Jan 11, 2022",
    comment: 35,
  },
];
